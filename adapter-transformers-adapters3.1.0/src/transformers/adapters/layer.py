...
from .composition import AdapterCompositionBlock, BatchSplit, Fuse, Parallel, Split, Stack, Aggregate
from .configuration import AdapterConfig
from .context import AdapterSetup, ForwardContext
import sys
from .modeling_sappa import Adapter, BertFusion, ParallelAdapter
...
class AdapterLayer(AdapterLayerBase, nn.Module):
    ...
    def adapter_stack(self, adapter_setup: Stack, hidden_states, input_tensor, layer_norm, lvl=0, use_sappa=True):
        """
        ...
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            # Case 5: We have a single adapter which is part of this module -> forward pass
                ...
                if not use_sappa:
                    ...
                    context = ForwardContext.get_context()
                    layer_output = adapter_layer(
                        hidden_states, residual_input=residual, x_pa=input_tensor,
                        output_gating=context.output_adapter_gating_scores
                    )
                    hidden_states, up, hidden_states_pa, up_pa = layer_output[0], layer_output[2], layer_output[3], \
                        layer_output[5]
                    self._store_gating_score(adapter_stack_layer, layer_output[-1])
                    # as this stack might be part of a fusion block, return the adapter up-projection output here
                    # together with the final output (with potential residuals & norms) if we reached the last block of the stack
                    if i == len(adapter_setup) - 1:
                        return hidden_states, up, input_tensor, hidden_states_pa, up_pa
        ...
        if not use_sappa:
            ...
            return hidden_states, None, input_tensor, hidden_states_pa, None
    ...
    def adapter_parallel(self, adapter_setup: Parallel, hidden_states, input_tensor, layer_norm, lvl=0):
        ...
        clone_input_tensor = input_tensor.clone()
        context = ForwardContext.get_context()
        ...
        # print(child_hidden_states.size())
        # hidden_states = torch.stack(children_hidden).sum(0)
    ...
    def adapter_aggregate(self, adapter_setup: Stack, hidden_states, input_tensor, layer_norm, lvl=0, gumbel=False):
        """
        Accumulate the output of each adapter in parallel and return the sum.
        """
        child = []
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            child.append(hidden_states)

        # If we got here, we either had another nested composition block
        # or no adapter was found. In both cases, we don't need to set the second return value for fusion
        return child[0], child[1]
    
    def adapter_layer_forward(self, hidden_states, input_tensor, layer_norm, use_sappa=True):
        """
        ...
        if adapter_setup is not None:
            ...
            if isinstance(adapter_setup, Stack):
                if not use_sappa:
                    ...
                    hidden_states_sa, _, input_tensor, hidden_states_pa, _ = self.adapter_stack(
                        adapter_setup, hidden_states, input_tensor, layer_norm
                    )
            ...
            elif isinstance(adapter_setup, Aggregate):
                hidden_states1, hidden_states2 = self.adapter_aggregate(adapter_setup, hidden_states, input_tensor, layer_norm)
                last_adapter = self.adapters[adapter_setup.last()]
                hidden_states = last_adapter.post_forward(hidden_states2, hidden_states1, input_tensor, layer_norm)
                return hidden_states
            else:
            ...
            if not use_sappa:
                ...
                hidden_states = last_adapter.post_forward(hidden_states_pa, hidden_states_sa, input_tensor, layer_norm, input_hidden_states)
