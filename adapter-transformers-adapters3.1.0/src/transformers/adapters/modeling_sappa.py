...
class Adapter(nn.Module):
    ...
    def __init__(
        ...
        self.original_ln_before_pa = False
        self.down_sample_pa = int(self.input_size // config["reduction_factor_pa"])
        self.original_ln_after = config["original_ln_after"]
        self.is_sa_alone = config["is_sa_alone"]
        self.is_pa_alone = config["is_pa_alone"]
        ...
        if self.add_layer_norm_before:
            ...
            seq_list_pa.append(self.adapter_norm_before)
        ...
        if config["phm_layer"]:
            ...
            seq_list_pa.append(PHMLayer(adapter_name, self.input_size, self.down_sample_pa, "down", config))
        else:
            seq_list.append(nn.Linear(self.input_size, self.down_sample))
            seq_list_pa.append(nn.Linear(self.input_size, self.down_sample_pa))

        # select non-linearity
        self.non_linearity = Activation_Function_Class(config["non_linearity"].lower())

        seq_list.append(self.non_linearity)
        seq_list_pa.append(self.non_linearity)

        # sequential adapter, first downproject, then non-linearity then upsample. In the forward pass we include the
        # residual connection
        self.adapter_down = nn.Sequential(*seq_list)
        self.adapter_down_pa = nn.Sequential(*seq_list_pa)

        # Up projection to input size
        if config["phm_layer"]:
            # Linear down projection of the input
            self.adapter_up = PHMLayer(adapter_name, self.down_sample, self.input_size, "up", config)
            self.adapter_up_pa = PHMLayer(adapter_name, self.down_sample_pa, self.input_size, "up", config)
        else:
            self.adapter_up = nn.Linear(self.down_sample, self.input_size)
            self.adapter_up_pa = nn.Linear(self.down_sample_pa, self.input_size)

        # Additional scaling factor (from He et al. (2021))
        if isinstance(config["scaling"], float):
            self.scaling = config["scaling"]
            self.scaling_pa = config["scaling_pa"]
        ...
        if config["init_weights"] == "bert":
            ...
            with torch.no_grad():
                nn.init.kaiming_uniform_(self.adapter_down_pa[0].weight, a=math.sqrt(5))
                nn.init.zeros_(self.adapter_up_pa.weight)
                nn.init.zeros_(self.adapter_down_pa[0].bias)
                nn.init.zeros_(self.adapter_up_pa.bias)
            if self.use_gating:
    ...
    def forward(self, x, residual_input, x_pa, output_gating=False):
        if not self.is_pa_alone:
            down = self.adapter_down(x)

            up = self.adapter_up(down)
            # up = up * self.scaling * gs[0]
            up = up * self.scaling
        ...
        # pa computation
        if not self.is_sa_alone:
            down_pa = self.adapter_down_pa(x_pa)

            up_pa = self.adapter_up_pa(down_pa)
            # up_pa = up_pa * self.scaling_pa * gs[1]
            up_pa = up_pa * self.scaling_pa

            output_pa = up_pa

            # apply layer norm if available
            if self.add_layer_norm_after:
                output_pa = self.adapter_norm_after(output_pa)
        if self.is_sa_alone:
            return output, down, up, output, down, up
        elif self.is_pa_alone:
            return output_pa, down_pa, up_pa, output_pa, down_pa, up_pa
        else:
            return output, down, up, output_pa, down_pa, up_pa

    def post_forward(self, hidden_states, input_hidden_states, input_tensor, layer_norm, original_input_hidden_states):
        """
        ...
        """
        # here hiddenstates is pa states, input hidden states is sa states
        if self.is_sa_alone:
            if self.original_ln_after:
                if layer_norm:
                    hidden_states = layer_norm(input_hidden_states + input_tensor)
                else:
                    hidden_states = input_hidden_states + input_tensor
        elif self.is_pa_alone:
            hidden_states = hidden_states + original_input_hidden_states
