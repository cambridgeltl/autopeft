...
from transformers.adapters import ConfigUnion, ParallelConfig, PrefixTuningConfig, LoRAConfig
import transformers.adapters.composition as ac
import numpy as np
...
class ModelArguments:
    ...
    prefix_length: int = field(
        default=64, metadata={"help": "prefix_length for the number of hidden units in the adapters."}
    )
    reduction_factor: int = field(
        default=64, metadata={"help": "Reduction factor for the number of hidden units in the adapters."}
    )
    adapter_name: Optional[str] = field(
        default=None, metadata={"help": "adapter config name"}
    )
...
def main():
    ...
    logger.info(f"model number of parameters before heads{model.num_parameters()}")
    ...
    # else:
    #     model.load_head(my_checkpoint+"/"+data_args.task_name)

    logger.info(f"model number of parameters before adapter{model.num_parameters()}")
    ...
    if adapter_args.train_adapter:
        ...
        if model_args.adapter_name == "pfeiffer":
            adapter_config = transformers.AutoPEFTConfig(reduction_factor=16, is_sa_alone=True)
        if model_args.adapter_name == "prefix":
            adapter_config = transformers.PrefixTuningConfig(prefix_length=10, bottleneck_size=800)
        if model_args.adapter_name == "parallel":
            adapter_config = transformers.AutoPEFTConfig(reduction_factor_pa=2, is_pa_alone=True)
        if model_args.adapter_name == "lora":
            adapter_config = transformers.LoRAConfig()
        if model_args.adapter_name == "mam":
            adapter_config = ConfigUnion(
                transformers.PrefixTuningConfig(prefix_length=30, bottleneck_size=800),
                transformers.AutoPEFTConfig(reduction_factor_pa=2, is_pa_alone=True),
                )
        if model_args.adapter_name == "unipelt":
            adapter_config = ConfigUnion(
                LoRAConfig(r=8, use_gating=True),
                PrefixTuningConfig(prefix_length=10, use_gating=True),
                transformers.AutoPEFTConfig(reduction_factor=16, use_gating=True, is_sa_alone=True),
            )

        print(model_args.adapter_name)

        if model_args.adapter_name == "sappa":
            adapter_config = ConfigUnion(
                transformers.PrefixTuningConfig(),
                transformers.AutoPEFTConfig(reduction_factor=4, reduction_factor_pa=8),
            )
            model.add_adapter('sappa', config=adapter_config)
            model.train_adapter('sappa')
        ...
        if "sappa" not in model_args.adapter_name:
    ...
    model_param_dict['heads'] = model_param_dict['w. heads'] - model_param_dict['model']
    model_param_dict['adapter'] = model_param_dict['w. heads & adapter'] - model_param_dict['w. heads']
    ...
    if data_args.resplit_dataset:
        print('original length of training dataset: ', len(raw_datasets["train"]))
